...
const screenForNames = (arr) =>
  arr
    .map((str) => str.match(/^(Mr|Mrs|Ms|Dr|)\..[A-Za-z]+$/g))
    .filter((item) => item)
    .flat();
...
const toTitleCase = (arr) =>
  arr.map((item) => item[0].toUpperCase() + item.slice(1));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
let biggerThanLuke = (arr) =>
  arr.reduce((a, b) => a + (b.mass > 77 ? " - " + b.name : ""), "").slice(3);
...
const detectTicTacToeWin = (board) => {
  ...
  for (let row = 0; row < board.length; row++) {
    let col = [];
    win = helper(board[row]);
    if (win) {
      return win;
    }
    for (let j = 0; j < board[row].length; j++) {
      col.push(board[j][row]);
      if (row === j) {
        dig.push(board[row][j]);
      }
    }
    win = helper(col);
