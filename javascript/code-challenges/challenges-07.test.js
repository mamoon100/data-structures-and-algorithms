...
const sortStarWarsCharacters = (starWarsArr) =>
  starWarsArr.sort((a, b) => Number(b.height) - Number(a.height), 0);
...
const splitFoods = (recipe) => {
  ...
  for (let i = 0; i < food.length; i++) {
    let item = food[i];
    for (let j = 0; j < size.length; j++) {
      item = item.includes(size[j])
        ? item.split(size[j])[item.split(size[j]).length - 1]
        : item;
    }
    result.push(item);
  }
...
const removeLastCharacters = (str, numberOfCharacters) =>
  numberOfCharacters > 0 ? str.slice(0, -1 * numberOfCharacters) : str;
...
const totalSumCSV = (str) => {
  let total = 0;
  str.split(",").forEach((item) => (total += Number(item)));
  return total;
};
