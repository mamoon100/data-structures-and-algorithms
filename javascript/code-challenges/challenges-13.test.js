...
const longestString = (arr) => {
  let copy = [...arr];
  return arr.indexOf(copy.sort((a, b) => b.length - a.length)[0]);
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const standardizePhoneNumbers = (arr) =>
  arr.map((item) =>
    item.split("(")[1].split(") ").join("").split("-").join("")
  );
...
const allHappy = (arr) => arr.every((item) => item.includes(":)"));
...
const findAnything = (arr, target) =>
  arr.filter((item) => item.includes(target));
...
const findEvery = (arr, target) => arr.every((item) => item.includes(target));
...
const unenrollBrook = (arr) =>
  arr.map((item) => item.filter((e) => !e.includes("Brook")));
...
const sortByDay = (arr) => {
  ...
  daysOfWeek.forEach((day) => {
    eventArray.push(arr.filter((item) => item.includes(day)));
  });
  return eventArray;
};
