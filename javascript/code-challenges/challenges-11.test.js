...
const count = (target, input) =>
  input
    .map((item) => item.filter((value) => value === target))
...
const divisibleByFiveTwoToThePower = (input) =>
  input.map((item) =>
    item
      .filter((value) => value % 5 === 0 && typeof value === "number")
      .map((a) => Math.pow(2, a))
  );
  ...
  data
    ...
    .reduce(
      (a, b, index) =>
        ...
        (index <
        data.filter(
          (item) => item.gender === "female" || item.gender === "male"
        ).length -
          1
          ? " "
          : ""),
...
let findShortest = (data) =>
  data.sort((a, b) => Number(a.height) - Number(b.height))[0].name;
