...
const toLastNames = (people) => {
  let newArr = [];
  people.map((e, i) => {
    newArr.push(`${e.firstName} ${e.lastName}`);
  });
  return newArr;
};
...
const returnNames = (arr) => {
  let newArr = [];
  arr.reduce((a, b) => newArr.push(b.name), "");
  return newArr;
};
...
const countNumberOfChildren = (arr) =>
  arr.reduce((a, b) => a + (b.children ? b.children.length : 0), 0);
/* ------------------------------------------------------------------------------------------------
...
const calculateAverage = (arr) => arr.reduce((a, b) => a + b / arr.length, 0);
...
const countPrimeNumbers = (arr) =>
  arr.reduce((a, b) => a + (isPrime(b) ? 1 : 0), 0);
...
// arr.reduce((a, b) => console.log(arr[0].stat.name));

const extractStat = (statName, arr) =>
  arr.reduce((a, b) => (a = statName === b.stat.name ? b : a), "");
