...
E.g. [4,2,7,5,9,2] -> 9
------------------------------------------------------------------------------------------------ */
const maxInArray = (arr) => arr.reduce((a, b) => (a = a > b ? a : b), 0);
...
const hasChildrenValues = (arr, character) =>
  arr.filter((item) => item.name === character)[0].children ? true : false;
...
const hasChildrenEntries = (arr, character) =>
  arr.filter((item) => item.name === character)[0]
    ? hasChildrenValues(arr, character)
    : false;
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach((item) => {
    ...
    survivors.push({
      ...
      members:
        (item.name ? 1 : 0) +
        (item.spouse ? (deceasedSpouses.includes(item.spouse) ? 0 : 1) : 0) +
        (item.children ? item.children.length : 0),
    });
  });
  return survivors;
